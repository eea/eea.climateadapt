	...
	private static String normalizeFileName( String fileName ) {
		...
		if (PropsValuesExt.LAYOUT_IMPORT_DL_FILE_NAME_NORMALIZATION_ENABLED) {
			if (!DLUtilExt.isValidFileName(fileName)) {
               if (_log.isWarnEnabled()) {
            	   _log.warn("Normalizing file name \"" + fileName + "\"");
               }

               newFileName = DLUtilExt.normalizeFileName(fileName);

               if (_log.isWarnEnabled()) {
            	   _log.warn("Normalized file name \"" + fileName + "\" as \"" + newFileName + "\"");
               }
			}
		}
		return newFileName;
					...
					if (!fileVersion.getUuid().equals(
						...
						importedFileEntry =
								DLAppLocalServiceUtil.updateFileEntry(
									...
									fileEntryTitle, fileEntry.getMimeType(),
									fileEntryTitle,
